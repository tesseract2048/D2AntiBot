...
BOOL QueryReg(char* data, char const *KeyName, char const *Name){
	...
	if(RegOpenKeyEx(HKEY_CURRENT_USER, KeyName, 0, KEY_READ, &hKey))
		return FALSE;
	if(RegQueryValueEx(hKey, Name, NULL, &type, (LPBYTE)data, &size))
	{
		RegCloseKey(hKey);
		return FALSE;
	}
...
BOOL SetReg(char const *KeyName, char const *Name, char const *Value){
	HKEY hKey;
	if(!RegOpenKeyEx(HKEY_CURRENT_USER, KeyName, 0, KEY_READ | KEY_WRITE, &hKey))
	{
		RegSetValueEx(hKey, Name, NULL, REG_SZ, (BYTE*)Value, len(Value));
		return TRUE;
	}
	return FALSE;
}
